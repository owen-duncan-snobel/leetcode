 ...
 * whiteboard to totally understand how the stack is storing every value
 * getting processed by the program.
 *
 * stack gets max depth scope
 * then the stack travels to the left based on the functions scope it recieved,
 ...
 * quickly in succesion. then it travels up to 3's scope which hasn't 
 * finished executing. it returns a 1 as part of the scope.
 * then it travels to 20 and gives that scope. 20 then goes down into 15's 
 * scope. 15 returns 0 twice for each null value in its scope. then it 
 * returns 1 to the 20's scope but 20 hasn't finished executing so 
 * then 20's function exectuion travels down to 7 and 7 returns a 1 
 * to 20's scope after it's two null scopes return 0. 20 then compares 1
 ...
 * and then the 20's scope which returned it 2. It selects 2 as being
